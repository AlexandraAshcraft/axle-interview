...
try {
  ...
  const updatedProvider = await db.query(
    `UPDATE providers SET ${queryStr} WHERE provider_id = ${req.body.provider_id};`,
  );
...
try {
  ...
  if (!provider_id)
    return next({
      ...
      status: 400,
      message: { err: 'Invalid request: Please include provider identifier.' },
    });
