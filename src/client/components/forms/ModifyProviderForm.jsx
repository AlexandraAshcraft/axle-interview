...
export const ModifyProviderForm = () => {
  const {
    currModifiedProvider,
    modifiedProviderOpen,
    setModifiedProviderOpen,
  } = useContext(StoreContext);

  const handleClickOpen = () => setModifiedProviderOpen(true);
  const handleClose = () => {
    setModifiedProviderOpen(false);
    reset();
  };

  const defaultValues = Object.values(currModifiedProvider);
  console.log('default provider values', defaultValues);
  ...
  const submitForm = async formData => {
    ...
    const updates = Object.entries(updateRequestBody).filter(
      ([key, value]) => value !== currModifiedProvider[key],
    );
    modifyProvider(currModifiedProvider.provider_id, updates);
  ...
  return (
    <div className='form' id='provider-form'>
      <Dialog open={modifiedProviderOpen} onClose={handleClose}>
